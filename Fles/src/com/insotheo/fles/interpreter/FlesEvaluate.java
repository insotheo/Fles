...
public class FlesEvaluate {
    ...
    public static FlesValue evalExpression(Module current, ASTNode node, VariableStack variables) throws Exception {
        if (node.getClass() == NumberNode.class) {
            ...
            return evalFunctionCall(current, funcCallNode, variables).getReturnValue();
        } else if (node.getClass() == BlockNode.class) {
            return evalBlock(current, ((BlockNode) node).getStatements(), variables).getReturnValue();
            ...
            FlesValue left = evalExpression(current, binaryNode.getLeft(), variables);
            FlesValue right = evalExpression(current, binaryNode.getRight(), variables);
            ...
            FlesValue val = new FlesValue(ValueType.Boolean, evalExpression(current, ((NotNode) node).getExpression(), variables).getData());
            return val.not();
        }
    ...
    public static BlockReturn evalBlock(Module current, List<ASTNode> nodes, VariableStack inputVariables) throws Exception {
        ...
        for (ASTNode node : nodes) {
            if (node.getClass() == FunctionCallNode.class) {
                ...
                evalFunctionCall(current, callNode, variables);
            } else if (node.getClass() == VariableNode.class) {
                ...
                if (assignmentNode.getIsJustCreated()) {
                    ...
                    FlesValue newVarValue = evalExpression(current, assignmentNode.getValue(), variables);
                    ...
                    FlesValue value = evalExpression(current, assignmentNode.getValue(), variables);
                ...
                FlesValue value = evalExpression(current, ((ReturnNode) node).getValue(), variables);
                ...
                evalBlock(current, ((BlockNode) node).getStatements(), variables);
            } else if (node.getClass() == IfBranch.class) {
                IfBranch branch = ((IfBranch) node);
                boolean finished = false;
                for (ConditionBlock block : branch.getBranches()) {
                    FlesValue expressionResult = evalExpression(current, block.getExpression(), variables);
                    ...
                    if (((boolean) expressionResult.getData())) {
                        evalBlock(current, block.getBody().getStatements(), variables);
                        finished = true;
                        break;
                    }
                }
    ...
    public static BlockReturn evalFunction(FlesFunction function) throws Exception {
        return evalBlock(function.getModule(), function.getStatements(), function.getParameters());
    }

    public static BlockReturn evalFunctionCall(Module current, FunctionCallNode callNode, VariableStack variables) throws Exception {
        List<FlesValue> arguments = new ArrayList<>();
        List<ASTNode> expressions = callNode.getArguments();
        for (ASTNode node : expressions) {
            FlesValue val = evalExpression(current, node, variables);
            arguments.add(val);
        }
        return InterpreterData.callFunction(current, callNode.getName(), arguments);
    }
